...
public class BinarySearch
{
    ...
    public int GetIndex(List<int> sortedList, int target)
    {
        ...
        while (start <= end)
        {
            ...
            {
                Console.WriteLine($"Binary search found target - {target}");
                Console.WriteLine($"Binary search count of operations equals: {operations}");
                return mid;
            }
    ...
    public int FindInsertionPlace(List<int> sortedList, int value)
    {
        int start = 0;
        int end = sortedList.Count - 1;
        int operations = 0;
